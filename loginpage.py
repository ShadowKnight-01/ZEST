...
class LoginPage(QWidget):
    def __init__(self, stack_manager):
        ...
        self.stack = stack_manager     # Save the stack manager for navigation
        self.init_ui()                 # Create and set up the user interface

    def init_ui(self):
        layout = QVBoxLayout()       # make a layout for the login page
        layout.setAlignment(Qt.AlignCenter)
        layout.setSpacing(15)

        title = QLabel("LogIn")      # display the page title
        title.setFont(QFont('Segoe UI', 25, QFont.Bold))
        title.setAlignment(Qt.AlignCenter)
        layout.addWidget(title)

        self.txt_login = QLineEdit()   # make a box for input username or email
        self.txt_login.setPlaceholderText("Username or Email")
        self.txt_login.setFixedWidth(320)

        self.txt_password = QLineEdit()# make a box for input password and hid it too
        self.txt_password.setPlaceholderText("Password")
        self.txt_password.setEchoMode(QLineEdit.Password)
        self.txt_password.setFixedWidth(320)

        btn_login = QPushButton("Log In")    # make a box for login and connect it to the function
        btn_login.setFixedWidth(320)
        btn_login.clicked.connect(self.auth_user)

        btn_goto_register = QPushButton("Don\'t have an account? Create Account")   # create a box to go to register page
        ...
        layout.addWidget(self.txt_login)    # add widhets to the layout
    ...
    def auth_user(self):
        ident = self.txt_login.text().strip()        # get the user login detail
        password = self.txt_password.text()

        try:
            result = supabase.table("users").select("*").or_(f"email.eq.{ident},username.eq.{ident}").execute()   # search for the user by email or username
            if not result.data:
                QMessageBox.warning(self, "Auth Error", "Email or Username not found, you may need to register.")
                return

            db_user = result.data[0]
            hashed_input = hashlib.blake2b(password.encode()).hexdigest()   # encrypt the enteres password

            if db_user["password"] != hashed_input: # check if the password is same, if same correct
                QMessageBox.warning(self, "Auth Error", "Wrong password, you may try again.")
                return

            # Save the user's information for the current session
            SESSION["user_id"] = db_user["user_id"]
            SESSION["username"] = db_user["username"]
            SESSION["full_name"] = db_user["full_name"]
            SESSION["gender"] = db_user["gender"]
            SESSION["interest"] = db_user["interest"]

            # Load the data and got to the profile page
